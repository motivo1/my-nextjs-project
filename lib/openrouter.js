...
// Non-streaming helper: returns the assistant's full reply as a string.
async function complete(systemPrompt, messages) {
  const res = await chatCompletion(systemPrompt, messages, false);
  const data = await res.json();
  return data.choices?.[0]?.message?.content || "";
}

export { chatCompletion, complete, isConfigured, OPENROUTER_MODEL };
